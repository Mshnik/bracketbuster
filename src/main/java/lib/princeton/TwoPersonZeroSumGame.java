 ...
 *  Dependencies: LinearProgramming.java System.out.java
 *
 ...
 * <p>
 * This implementation solves an <em>m</em>-by-<em>n</em> two-person zero-sum game by reducing it to
 * a linear programming problem. Assuming the payoff matrix <em>A</em> is strictly positive, the
 ...
 *  </pre></blockquote>
 * <p>
 * If the payoff matrix <em>A</em> has any negative entries, we add the same constant to every entry
 * so that every entry is positive. This increases the value of the game by that constant, but does
 * not change solutions to the two-person zero-sum game.
 * <p>
 * This implementation is not suitable for large inputs, as it calls a bare-bones linear programming
 * solver that is neither fast nor robust with respect to floating-point roundoff error.
 * <p>
